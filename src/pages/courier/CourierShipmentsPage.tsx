...
import { Phone, MapPin, Search, CheckCircle, Loader2 } from 'lucide-react';
...
const CourierShipmentsPage: React.FC = () => {
  ...
  }, [shipments, period, statusFilter, search]);

  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" className="space-y-4" dir="rtl">
      <h2 className="text-lg font-bold text-right">{t.myShipments}</h2>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder={t.search} value={search} onChange={e => setSearch(e.target.value)} className="ps-9 rounded-xl w-full text-right" />
        </div>
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-full sm:w-32 rounded-xl text-right"><SelectValue placeholder={t.allStatuses} /></SelectTrigger>
          <SelectContent>
            ...
            {filtered.map((s, i) => (
              <motion.div key={s.id} custom={i} variants={cardVariants} initial="initial" animate="animate"
                ...
                <div className="p-4 text-right">
                  ...
                  <div className="flex items-center gap-1 text-xs text-muted-foreground mb-2 justify-end">
                    {s.city} <MapPin className="w-3 h-3" />
                  </div>
